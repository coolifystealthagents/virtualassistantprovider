...
| `/services/marketing-assistants` | How can a team prepare content work while the owner keeps approval and publishing authority? | `/blog/virtual-assistant-content-operations-philippines` | Verified absent — next candidate | Fresh generated route-local content has no `/services/marketing-assistants` href. A separately scoped, data-owned handoff may add one only after checking the route record and shared renderer. Keep claims, final publishing, and budget decisions with the owner. |
