...
export default async function ServicePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><Header/><main>
    <JsonLd data={schema} />
    <section className="hero"><div className="container two"><div><p className="eyebrow">Service guide</p><h1>{service.h1}</h1><p className="lead">{service.excerpt}</p><p>This page helps you scope the role before you talk to a provider, freelancer, or recruiter.</p><a className="btn" href="/contact">Get my role plan</a></div><div className="card"><h3>Best for</h3>{service.bestFor.map((item)=><span className="pill" key={item}>{item}</span>)}</div></div></section>
