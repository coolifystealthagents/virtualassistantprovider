...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return { title: post?.title || 'Guide', description: post?.excerpt, alternates: post ? { canonical: site.url + "/blog/" + post.slug } : undefined, openGraph: post?.published ? { type: 'article', url: site.url + "/blog/" + post.slug, publishedTime: post.published, modifiedTime: post.updated ?? post.published, images: [{ url: site.url + post.featuredImage, alt: post.title }] } : undefined };
}
...
export default async function Post({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const schema = {
    ...
    '@graph': [
      { '@type': 'Article', headline: post.title, description: post.excerpt, datePublished: post.published, dateModified: post.updated ?? post.published, author: { '@type': 'Organization', name: site.brand }, publisher: { '@type': 'Organization', name: site.brand, url: site.url }, mainEntityOfPage: site.url + "/blog/" + post.slug, image: site.url + post.featuredImage, citation: post.sources.map((source) => source.url), hasPart: post.sections.map((section, index) => ({ '@type': 'WebPageElement', position: index + 1, name: section.heading })) },
